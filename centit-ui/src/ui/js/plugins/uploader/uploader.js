...
(function(){
	...
	StreamUploader.prototype = {
		...
		streamUpload: function(pos){
			...
			var range = "bytes "+ pos + "-"+ (pos + blob.size-1) + "/" + this.get("size");
			this.preTime = (new Date).getTime();
	...
	Main.prototype = {
		...
		completeUpload : function(info) {
			this.get("onComplete") ? this.get("onComplete").call(this, info) : this.onComplete(info);
		...
		upload : function(index) {
			...
			// 修改为本地获取文件md5
			FileMD5(file.file, {debug: false})
				.then(function(token) {
					bStreaming && bFileSlice ? (self.uploadFile(file, uploadURL, token, "resumeUpload"))
	...
	window.Uploader = window.Stream = Main; /** window.Uploader是SWF组件的关键字(保留) */
	window.onbeforeunload = function() {  if (uploading) return "正在上传文件。";  }
})();